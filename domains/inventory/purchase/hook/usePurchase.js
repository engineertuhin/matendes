...
import { purchaseSearchTemplate, toolSearchTemplate } from "@/utility/templateHelper";
...
export const usePurchase = () => {
  ...
  const { data: purchasesData, refetch, isFetching } = useFetchPurchasesQuery();
  ...
  const actions = {
    onCreate: async (data) => {
      try {
        ...
        let payload = normalizeSelectValues(other, ["supplier_id"]);
    
        // Set default purchase status as string
        payload.purchase_status = payload.purchase_status?.toString() ?? "completed";
        ...
        const details = (payload.details || []).map((d) => ({
          ...
          net_total_price: Number((d.net_total_price ?? d.total_price) || 0),
          warehouse_id: d.warehouse_id?.value ?? d.warehouse_id ?? null,
        }));
    
        // Validate details
        const invalidDetails = details.filter((d) => !d.tool_id || d.quantity < 1);
    ...
    onUpdate: async (data) => {
      try {
        ...
        const response = await updatePurchase({ id, ...payload }).unwrap();
        if (response) {
    ...
    onEdit: (item) => {
      ...
      const total_tool = (item.details || []).reduce((sum, d) => sum + Number(d.quantity || 0), 0);
